...
const handleDelete=(id)=>{
  ...
  .then((res)=> res.json())
  .then((data)=>{if (data.acknowledged === true) {
...
<div className="search-box p-2 text-center mb-2 sm:flex md:block ">
<input 
    ...
    <div class="rounded-t mb-0 px-4 py-3 border-0">
      <div class="flex flex-wrap items-center">
        ...
        <div class="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
          <button class="bg-indigo-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">POST A NEW JOB</button>
        </div>
      </div>
    </div>
    ...
    <div class="block w-full overflow-x-auto">
      <table class="items-center bg-transparent w-full border-collapse ">
        <thead>
         ...
         {
          isLoading ? (<div className='flex items-center justify-centerh-20'><p>Loading.....</p></div>):(
             <tbody>
            {
                jobs.map((job,index) =>(
                     <tr key={index}>
